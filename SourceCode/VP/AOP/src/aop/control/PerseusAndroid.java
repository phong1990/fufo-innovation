...
public class PerseusAndroid extends Activity implements OnClickListener, OnItemClickListener, OnCheckedChangeListener {
	...
	//00:08:CA:B3:6C:DA  00:12:02:15:60:12
	//-- debugging --//	
	String 						LOG_TAG 				= "FUFO";
	//-- GUI --//
	...
	public Button                      fwd,bck,rgt,lft,up,down,start;     
	...
	private View createTabContent1()
	{
    	...
    	lbBottom.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
     	...
     	LinearLayout panelIP = new LinearLayout(context);
     	panelIP.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT,LayoutParams.WRAP_CONTENT));
     	panelIP.setOrientation(LinearLayout.HORIZONTAL);
     	panelIP.setGravity(Gravity.BOTTOM);
     	panelIP.setGravity(Gravity.CENTER_VERTICAL);
     	
     	TextView lbIPserver = new TextView(context);
     	lbIPserver.setText("Server IP:");
     	lbIPserver.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
        panel.addView(lbIPserver);
     	
     	
     	panel.addView(panelIP);
	    return panel;
	}
	    ...
	    click = new Thread(){
		    public void run(){
		        ...
		        if (cmdId == idTab2STT)
		        {
		            if (m_btSck != null)
		                try {
		                    m_btSck.getOutputStream().write('f');
		                } catch (IOException e) {
		                    // TODO Auto-generated catch block
		                    e.printStackTrace();
		                }
    ...
    public void onCheckedChanged(RadioGroup arg0, int arg1) {
        ...
        if(arg0.getCheckedRadioButtonId() == 5555){
          ...
          //  writeToFile(b);
           try {
               if(m_btSck != null)
            m_btSck.getOutputStream().write('c');
           ...
           start.setEnabled(false);
            ...
            try {
                if(m_btSck != null)
                m_btSck.getOutputStream().write('n');
            } catch (IOException ex) {
                // TODO Auto-generated catch block
                ex.printStackTrace();
            }
