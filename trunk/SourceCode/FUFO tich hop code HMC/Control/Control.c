...
void controlFUFO(void){
	if(Forward == 1){
		setSetpoint(3.59,-23.16,-1,docao);
	} else if(Backward == 1){
		setSetpoint(3.59,-2.77,-1,docao);
	} else if(Left == 1){
		setSetpoint(-5.41,-14.16,-1,docao);
	} else if(Right == 1){
		setSetpoint(10.59,-14.16,-1,docao);
	} else setSetpoint(3.59,-14.16,-1,docao);
}
...
//	fufoSendCharUART(',');
	...
	fufoSendIntUART((int)(xuatPsi*1000));
	fufoSendCharUART(',');

	if(xuatHMC < 0){
		fufoSendCharUART('-');
	}
	fufoSendIntUART((int)(xuatHMC*10));
//	fufoSendCharUART(',');
